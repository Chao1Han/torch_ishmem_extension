...
#
# ISHMEM version of combine.py for Intel XPU devices
...
@torch.library.impl(lib, "all_to_all_vdev_2d_copy", "XPU")
def _all_to_all_vdev_2d_copy_xpu(
    input: torch.Tensor,
...
class AllToAllVDev2dOffset(torch.autograd.Function):
    ...
    def forward(  # type: ignore[no-untyped-def]
        ...
        """
        # Shuffle input to output using ISHMEM
    ...
    def backward(  # type: ignore[no-untyped-def]
        ...
        # Shuffle gradients back to the input using ISHMEM
...
def test_token_combine() -> None:
    """Test TokenCombiner with ISHMEM on Intel XPU devices."""
    import os
    ...
    # ISHMEM specific environment setup - BEFORE dist.init_process_group
    ...
    if enable_profile:
        ...
        prof = nullcontext()
    print(f"[Rank {rank}] Starting test_token_combine...", flush=True)
    
    # Init with xccl backend FIRST - this initializes MPI
    dist.init_process_group(backend="xccl", rank=rank, world_size=world_size)
    
    # Import ISHMEM extension AFTER dist.init_process_group
    # This is critical because ISHMEM requires MPI to be initialized
    import torch_ishmem_extension as ishmem_ext
    
    device = "xpu"
    # torch.xpu.set_device(device)

    # Set ISHMEM as SymmMem backend
    ...
    hid = 128  # Use smaller hidden dim for testing
    ...
    with prof:
        ...
        print(f"[Rank {rank}] Dispatching tokens first...", flush=True)
        ...
        # Don't compile for initial testing
        # compiled_combiner = torch.compile(combiner, fullgraph=True)
        ...
        print(f"[Rank {rank}] Testing TokenCombiner...", flush=True)
    ...
    # Check gradients
    print(f"[Rank {rank}] Checking gradients...", flush=True)
    ...
    for i, (split, offset) in enumerate(zip(out_splits, out_offsets)):
        ...
        expected = torch.empty(split, hid, device=device).fill_(dst_rank + 1)
        if not torch.allclose(grad_chunk, expected, rtol=1e-4, atol=1e-4):
            print(f"[Rank {rank}] Gradient mismatch at chunk {i}: "
                  f"got {grad_chunk[0].tolist()[:5] if split > 0 else []}, "
                  f"expected {expected[0].tolist()[:5] if split > 0 else []}", flush=True)

    dist.destroy_process_group()
    print(f"[Rank {rank}] ✅ TokenCombiner test passed!")


if __name__ == "__main__":
    # To run this test, use the following command:
    #   mpirun -n 4 python combine.py
    test_token_combine()
