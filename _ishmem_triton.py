...
class NvshmemLibFinder:
    """
    A class to find paths to the ISHMEM device libraries.

    Environment variable:

    `ISHMEM_LIB_DIR` (Optional[str]): The directory where the upstream ISHMEM
    device library is located.
    """
    ...
    def find_device_library(cls) -> str:
        """
        Find the path to the upstream ISHMEM device library.

        Returns:
            str: The path to the upstream device bitcode.
        """
        ...
        user_lib_dir = os.environ.get("ISHMEM_LIB_DIR")
        if user_lib_dir is not None:
            lib_path = os.path.join(user_lib_dir, "libishmem_device.bc")
            if os.path.exists(lib_path):
                cls.found_device_lib_path = lib_path
                return lib_path
            raise RuntimeError(
                f"ISHMEM device library not found at specified path: {user_lib_dir}"
            )
        ...
        if ishmem_root:
            paths.append(os.path.join(ishmem_root, "lib"))
        ...
        for path in paths:
            device_lib = os.path.join(path, "libishmem_device.bc")
            if os.path.exists(device_lib):
                cls.found_device_lib_path = device_lib
                return device_lib

        raise RuntimeError(f"ISHMEM device library not found. Searched: {paths}")
    ...
    def find_extension_device_library(cls) -> str:
        ...
        candidates = [
            os.path.join(os.path.dirname(__file__), "libishmem_device.bc"),
        ]
        ...
        raise RuntimeError(
            ...
            f"{candidates}. Build the repository so libishmem_device.bc exists, "
    ...
    def find_extern_libraries(cls) -> dict[str, str]:
        return {
            "libishmem_device": cls.find_device_library(),
            "libishmem_triton_wrapper": cls.find_extension_device_library(),
        }
...
def requires_nvshmem(  # type: ignore[no-untyped-def]
    ...
    If you would like to specify a path to the ISHMEM device library other
    than standard search locations, you can use the following environment
    variable:
    ```
        export ISHMEM_LIB_DIR=/path/to/ishmem/lib
    ```
    """
    ...
    # Find the upstream ISHMEM device library and the local Triton wrapper bitcode.
...
if has_triton():
    ...
    def putmem_block_extern_wrapper(dest, source, size_bytes, pe, _semantic=None):  # type: ignore[no-untyped-def]
        ...
        return core.extern_elementwise(
            ...
            [dest, source, size_bytes, pe],
            {
                ...
                ): ("ishmemx_putmem_work_group_wrapper", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
        ...
        return core.extern_elementwise(
            ...
            [dest, source, size_bytes, pe],
            {
                (
                    ...
                    core.dtype("int64"),  # size in bytes
                    core.dtype("int32"),  # pe number
                ): ("ishmemx_getmem_work_group_wrapper", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
    ...
    def get_nbi(dest, source, nelems, pe):  # type: ignore[no-untyped-def]
        ...
        tl.static_assert(dest.type == source.type)
        nbytes = nelems * dest.type.element_ty.itemsize
        return getmem_nbi_block_extern_wrapper(
            dest.to(tl.int64), source.to(tl.int64), nbytes.to(tl.int64), pe
        )
        ...
        return core.extern_elementwise(
            ...
            [dest, source, size_bytes, pe],
            {
                (
                    ...
                    core.dtype("int64"),  # size in bytes
                    core.dtype("int32"),  # pe number
                ): ("ishmemx_getmem_nbi_work_group_wrapper", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
        ...
        return core.extern_elementwise(
            ...
            [dst, src, size_bytes, signal, sig_val, sig_op, pe],
            {
                (
                    ...
                    core.dtype("int32"),
                    core.dtype("int32"),
                ): ("ishmemx_putmem_signal_work_group_wrapper", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
        ...
        return core.extern_elementwise(
            ...
            [ivar, cmp, cmp_val],
            {
                (
                    ...
                    core.dtype("int32"),
                    core.dtype("int32"),
                ): ("ishmem_int_wait_until", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
        ...
        return core.extern_elementwise(
            ...
            [signal, cmp, cmp_val],
            {
                (
                    ...
                    core.dtype("int32"),
                    core.dtype("uint64"),
                ): ("ishmem_signal_wait_until", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
        ...
        return core.extern_elementwise(
            ...
            [sig_addr, signal, sig_op, pe],
            {
                (
                    ...
                    core.dtype("int32"),
                    core.dtype("int32"),
                ): ("ishmemx_signal_op_wrapper", core.dtype("int32")) # TBD: missed this API in ishmem
            },
            is_pure=False,
            _semantic=_semantic,
        )
    ...
    def barrier_all(_semantic=None):  # type: ignore[no-untyped-def]
        ...
        return core.extern_elementwise(
            ...
            "",
            [],
            {(): ("ishmem_barrier_all", core.dtype("int32"))},
            is_pure=False,
            _semantic=_semantic,
        )
    ...
    def sync_all(_semantic=None):  # type: ignore[no-untyped-def]
        ...
        return core.extern_elementwise(
            ...
            "",
            [],
            {(): ("ishmem_sync_all", core.dtype("int32"))},
            is_pure=False,
            _semantic=_semantic,
        )
        ...
        return core.extern_elementwise(
            ...
            [team, dest, source, size_bytes],
            {
                (
                    ...
                    core.dtype("int64"),  # source ptr
                    core.dtype("int64"),  # size in bytes
                ): ("ishmemx_alltoallmem_work_group_wrapper", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
    ...
    def broadcastmem_block_extern_wrapper(
        ...
        return core.extern_elementwise(
            ...
            [team, dest, source, size_bytes, pe_root],
            {
                ...
                ): ("ishmemx_broadcastmem_work_group_wrapper", core.dtype("int32"))
            },
            is_pure=False,
            _semantic=_semantic,
        )
    ...
    def reduce_extern_wrapper(
        ...
        nvshmem_func = f"ishmem_{nvshmem_typename}_{op_name}_reduce"
