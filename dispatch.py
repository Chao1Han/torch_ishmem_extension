...
#
# ISHMEM version of dispatch.py for Intel XPU devices
...
@torch.library.impl(lib, "all_to_all_vdev_2d_offset_copy", "XPU")
def _all_to_all_vdev_2d_offset_copy_xpu(
    input: torch.Tensor,
...
def test_token_dispatch() -> None:
    """Test TokenDispatcher with ISHMEM on Intel XPU devices."""
    import os
    ...
    if enable_profile:
        ...
        prof = nullcontext()
    print(f"[Rank {rank}] Starting test_token_dispatch...", flush=True)
    ...
    with prof:
        ...
        # compiled_dispatcher = torch.compile(dispatcher, fullgraph=True)
        ...
        for i in range(iters):
            print(f"[Rank {rank}] Iteration {i}", flush=True)
    ...
    for i, split in enumerate(in_splits.tolist()):
        ...
        if not torch.allclose(grad_chunk, expected, rtol=1e-4, atol=1e-4):
            print(f"[Rank {rank}] Gradient mismatch at chunk {i}: "
                  f"got {grad_chunk[0].tolist()[:5]}, expected {expected[0].tolist()[:5]}", flush=True)
        start += split

    dist.destroy_process_group()
    print(f"[Rank {rank}] ✅ TokenDispatcher test passed!")


if __name__ == "__main__":
    # To run this test, use the following command:
    #   mpirun -n 4 python dispatch.py
    test_token_dispatch()
